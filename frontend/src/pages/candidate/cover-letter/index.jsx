...
export default function AICoverLetter() {
  const { user } = useAuth();
  const [showPreviousLetters, setShowPreviousLetters] = useState(false);
    ...
    <>
      ...
      <DashboardLayout>
        <div className="py-8">
          {/* Top Intro Section */}
          <div className="mb-8">
            <div className="rounded-lg border border-zinc-950/10 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 shadow-xs mb-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h1 className="text-2xl font-semibold text-zinc-950">AI Cover Letter</h1>
                    <Badge color="amber">Pro Feature</Badge>
                  </div>
                  <p className="text-sm text-zinc-600 leading-relaxed">
                    Maximize your chances with personalized cover letters generated by AI and reviewed by human (Platinum packages)
                  </p>
                </div>
              </div>
            </div>

            {/* Two Column Grid: Previous Letters & Generate New */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* View Previous Letters Card */}
              <div 
                onClick={() => setShowPreviousLetters(!showPreviousLetters)}
                className="group cursor-pointer rounded-lg border border-zinc-950/10 bg-white p-6 shadow-xs hover:shadow-sm transition-all hover:border-blue-300 h-full flex flex-col justify-between"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-zinc-100 group-hover:bg-blue-100 rounded-lg transition-colors">
                      <ClipboardDocumentIcon className="h-6 w-6 text-zinc-600 group-hover:text-blue-600 transition-colors" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-zinc-950 mb-1">Previous Generated Letters</h3>
                      <p className="text-sm text-zinc-500">
                        {previousLetters.length === 0 
                          ? 'No cover letters generated yet' 
                          : `${previousLetters.length} cover letter${previousLetters.length !== 1 ? 's' : ''} available`
                        }
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <ChevronRightIcon 
                      className={`h-5 w-5 text-zinc-400 transition-transform ${showPreviousLetters ? 'rotate-90' : ''}`} 
                    />
                  </div>
                </div>
              </div>

              {/* Generate New Letter Card */}
              <div 
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-zinc-100 group-hover:bg-blue-100 rounded-lg transition-colors">
                      <SparklesIcon className="h-6 w-6 text-zinc-600 group-hover:text-blue-600 transition-colors" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-zinc-950 mb-1">Generate New Letter</h3>
                      <p className="text-sm text-zinc-500">
                        Create a personalized cover letter with AI
                      </p>
                    </div>
                  </div>
                  <ChevronRightIcon className="h-5 w-5 text-zinc-400 group-hover:text-blue-600 transition-colors" />
                </div>
              </div>
            </div>
          </div>

          {/* Previous Letters Table */}
          {showPreviousLetters && (
            <div className="mb-8 rounded-lg border border-zinc-950/10 bg-white p-6 shadow-xs">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-zinc-950">Previous Generated Letters</h2>
              ...
              {loadingLetters ? (
                ...
                <div className="text-center py-8 text-zinc-500">
                  <DocumentTextIcon className="h-12 w-12 mx-auto mb-4 text-zinc-300" />
                  <p>No cover letters generated yet</p>
                  <Button onClick={handleStartWizard} color="blue" className="mt-4">
                  ...
                  <TableBody>
                    {previousLetters.map((letter) => (
                      <TableRow key={letter.id}>
                        ...
                        <TableCell className="font-medium">{letter.role}</TableCell>
                        <TableCell>{letter.company}</TableCell>
                        <TableCell>
                          <Badge color="zinc">{letter.template}</Badge>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Button
